...
export default function OverviewPage() {
  ...
  const rows = useMemo(() => {
    return customers
      ...
      .slice(0, 8);
  }, [customers, getOrder, orderTotals]);
    ...
    <div className="yp-rise">
      ...
      <PageHeading
        ...
        action={
          <button
            type="button"
            className="inline-flex h-9 items-center gap-2 rounded-full bg-surface px-3.5 text-[13px] font-medium text-ink ring-1 ring-hairline transition-colors hover:bg-surface-2"
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Ciro" value={formatCurrency(metrics.amount)} delta={periodDelta("revenue")} icon={Wallet} />
      ...
      >
        {/* Başlık satırı */}
        <div className="hidden grid-cols-[1.6fr_1fr_0.9fr_0.7fr_0.9fr_auto] items-center gap-4 px-4 py-2 text-[12px] font-medium text-ink-3 sm:grid">
          <span>Bayi</span>
          <span>Şoför</span>
          <span>Durum</span>
          ...
          {rows.map(({ customer, order, totals }) => {
            ...
            return (
              <button
                ...
                className="grid w-full grid-cols-[1fr_auto] items-center gap-4 rounded-[12px] px-4 py-2.5 text-left transition-colors hover:bg-surface-2 sm:grid-cols-[1.6fr_1fr_0.9fr_0.7fr_0.9fr_auto]"
              >
                ...
                <span className="hidden truncate text-[13px] text-ink-2 sm:block">{driver?.name ?? "—"}</span>
